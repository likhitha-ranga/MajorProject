...
def estimate_hba1c(age, bmi, hypertension, heart_disease, smoking_history):
    return round(4.8 + 0.05 * bmi + 0.1 * (age / 50) + 0.4 * hypertension + 0.3 * heart_disease, 1)

def estimate_glucose(age, bmi, hypertension, heart_disease, smoking_history):
    return round(85 + 2.5 * bmi + 0.4 * age + 10 * hypertension + 8 * heart_disease)

def diabetes_page():
    st.subheader('Diabetes Prediction')
    st.markdown("**Enter known values. If some are unknown, the app will estimate them.**")
    ...
    st.markdown("### BMI or Height & Weight")
    know_bmi = st.radio("Do you know your BMI?", ['Yes', 'No'])
    if know_bmi == 'Yes':
        bmi = st.number_input('Enter BMI (10.0 - 70.0)', min_value=10.0, max_value=70.0)
    else:
        ...
        if bmi:
            ...
            st.warning('BMI could not be calculated.')

    know_hba1c = st.radio("Do you know your HbA1c Level?", ['Yes', 'No'])
    hba1c_level = st.number_input('HbA1c Level (%)', min_value=3.0, max_value=15.0) if know_hba1c == 'Yes' else None

    know_glucose = st.radio("Do you know your Blood Glucose Level?", ['Yes', 'No'])
    blood_glucose_level = st.number_input('Blood Glucose Level (mg/dL)', min_value=50.0, max_value=500.0) if know_glucose == 'Yes' else None

    if st.button("Predict Diabetes"):
        if bmi is None:
            st.error("Please enter valid weight and height to calculate BMI or enter BMI directly.")
            return

        if hba1c_level is None:
            hba1c_level = estimate_hba1c(age, bmi, hypertension, heart_disease, smoking_history)
            st.info(f'Estimated HbA1c Level: {hba1c_level}%')
        else:
            st.success(f'Using provided HbA1c Level: {hba1c_level}%')

        if blood_glucose_level is None:
            blood_glucose_level = estimate_glucose(age, bmi, hypertension, heart_disease, smoking_history)
            st.info(f'Estimated Blood Glucose Level: {blood_glucose_level} mg/dL')
        else:
            st.success(f'Using provided Blood Glucose Level: {blood_glucose_level} mg/dL')
        ...
        threshold = 0.15
        diab_prediction = 1 if proba > threshold else 0

        if diab_prediction == 1:
            if blood_glucose_level < 140:
                stage = 'Pre-diabetes'
                symptoms = (
                    "You may experience mild symptoms such as increased thirst, "
                    "frequent urination, and slight fatigue. However, pre-diabetes often "
                    "has no obvious symptoms."
                )
                precautions = (
                    "Lifestyle changes are crucial at this stage. Maintain a balanced diet "
                    "low in processed sugars and refined carbs, increase physical activity, "
                    "lose excess weight if applicable, and monitor your blood glucose levels regularly."
                )
            elif blood_glucose_level < 200:
                stage = 'Type 2 Diabetes'
                symptoms = (
                    "Common symptoms include excessive thirst, frequent urination, "
                    "blurred vision, slow-healing sores, and fatigue. Some patients may "
                    "also experience tingling or numbness in the hands or feet."
                )
                precautions = (
                    "Management includes adhering to a diabetic-friendly diet, regular "
                    "exercise, maintaining a healthy weight, and taking medications or insulin "
                    "as prescribed by your healthcare provider. Regular monitoring of blood sugar "
                    "levels and routine medical checkups are essential."
                )
            else:
                stage = 'Type 1 Diabetes'
                symptoms = (
                    "Typical symptoms include rapid unexplained weight loss, extreme thirst, "
                    "frequent urination, fatigue, and sometimes nausea or vomiting. This condition "
                    "requires prompt medical attention."
                )
                precautions = (
                    "Type 1 diabetes requires insulin therapy to control blood sugar levels. "
                    "Close monitoring, a healthy diet, and regular consultations with your healthcare "
                    "team are necessary to prevent complications."
                )
        else:
            stage = 'No Diabetes'
            symptoms = (
                "Your test results do not indicate diabetes. However, it is important to maintain "
                "a healthy lifestyle to prevent future risk."
            )
            precautions = (
                "Continue eating a balanced diet, engaging in regular physical activity, "
                "maintaining a healthy weight, and scheduling routine health checkups."
            )
...
def heart_page():
    ...
    user_inputs['Resting BP'] = st.number_input('Resting Blood Pressure (mm Hg) (80-250)', min_value=80, max_value=250, help="Normal ~120 mm Hg. High >140 mm Hg indicates hypertension.")
...
def main_app():
    ...
    if st.session_state.logged_in:
        ...
        if selected == "Diabetes":
            ...
            st.session_state.logged_in = False
            st.session_state.username = None
            st.session_state.page = 'home'  # Reset to home
